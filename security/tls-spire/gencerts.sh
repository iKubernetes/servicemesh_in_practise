...
if [ ! -e "$CA_KEY" -o ! -e "$CA_CERT" ]; then
    ...
    cp $CERT_DIR/CA/ca.crt spire-server/ca.crt
fi

read -p "Certificate Name and Certificate Extenstions(envoy_server_cert/envoy_client_cert/spire_agent): " CERT EXT
while [ -n "$CERT" -a -n "$EXT" ]; do
	...
	cp $CERT_DIR/$CERT/server.crt $CERT_DIR/$CERT/server.key $CERT/
    elif [ "$EXT" == "envoy_client_cert" ]; then 
        openssl_req $CERT_DIR/$CERT client "/CN=$CERT"
        openssl_sign $CERT_DIR/CA/ca.crt $CERT_DIR/CA/ca.key $CERT_DIR/$CERT client $EXT
	cp $CERT_DIR/$CERT/client.crt $CERT_DIR/$CERT/client.key $CERT/
    else
        openssl_req $CERT_DIR/$CERT agent "/CN=$CERT"
        openssl_sign $CERT_DIR/CA/ca.crt $CERT_DIR/CA/ca.key $CERT_DIR/$CERT agent $EXT
	cp $CERT_DIR/$CERT/agent.crt $CERT_DIR/$CERT/agent.key $CERT/
    fi
    read -p "Certificate Name and Certificate Extenstions(envoy_server_cert/envoy_client_cert/spire_agent): " CERT EXT
done
